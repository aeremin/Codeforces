...
class Polynomial {
  public:
    ...
    int formal_degree() const { return coefficients_.size() - 1; }

    T get_coefficient(int d) const { return (d > formal_degree()) ? T(0) : coefficients_[d]; }
    ...
    const T& coefficient(int d) const { return coefficients_[d]; }

    T& coefficient(int d) { return coefficients_[d]; }
    ...
    bool operator!=(const Polynomial<T>& other) const { return !operator==(other); }
