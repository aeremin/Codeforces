...
class BipartitionChecker {
  public:
    BipartitionChecker(const UndirectedGraph<PerEdgeData, PerVertexData>& graph)
        : graph_(graph), part_(graph_.num_vertices(), -1) {
...
template <class PerEdgeData, class PerVertexData>
BipartitionChecker<PerEdgeData, PerVertexData> makeBipartitionChecker(
    const UndirectedGraph<PerEdgeData, PerVertexData>& graph) {
