...
void Solver611F::run() {
    ...
    for (int64_t t = 1; t <= pattern.length(); ++t) {
        ...
        if (c == 'U') {
            ++y;
            if (y >= timesToTop.size())
                timesToTop.push_back(t);
        }
        if (c == 'D') {
            --y;
            if (-y >= timesToBottom.size())
                timesToBottom.push_back(t);
        }
        if (c == 'R') {
            ++x;
            if (x >= timesToRight.size())
                timesToRight.push_back(t);
        }
        if (c == 'L') {
            --x;
            if (-x >= timesToLeft.size())
                timesToLeft.push_back(t);
        }
    }
    ...
    for (int i = 0; i < width; ++i)
        totalHorizontalTimePerLine =
            smartSummator(min(timesToLeft[i + 1], timesToRight[width - i]), totalHorizontalTimePerLine);
