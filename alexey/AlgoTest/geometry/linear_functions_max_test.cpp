...
TEST( LinearFunctionsMaximumTest, AddingNotImportantFunction ) {
    ...
    EXPECT_EQ( 1, lfm.get_value( -2 ) );
    EXPECT_EQ( 2, lfm.get_value( -1 ) );
    EXPECT_EQ( 3, lfm.get_value( 0 ) );
    EXPECT_EQ( 4, lfm.get_value( 1 ) );
    EXPECT_EQ( 7, lfm.get_value( 2 ) );
    EXPECT_EQ( 10, lfm.get_value( 3 ) );
}
...
TEST( LinearFunctionsMaximumTest, NotPreciseIntersection ) {
    ...
    EXPECT_EQ( 3, lfm.get_value( 0 ) );
    EXPECT_EQ( 4, lfm.get_value( 1 ) );
    EXPECT_EQ( 6, lfm.get_value( 2 ) );
    EXPECT_EQ( 9, lfm.get_value( 3 ) );
}
...
TEST(LinearFunctionsMaximumTest, SameSlope) {
    ...
    EXPECT_EQ(3, lfm.get_value(-1));
    EXPECT_EQ(4, lfm.get_value(0));
    EXPECT_EQ(5, lfm.get_value(1));
    EXPECT_EQ(6, lfm.get_value(2));
    EXPECT_EQ(7, lfm.get_value(3));
}
...
TEST( LinearFunctionsMaximumTest, RandomizedTestComparedWithBruteForceInt ) {
...
TEST(LinearFunctionsMaximumTest, RandomizedTestComparedWithBruteForceDouble) {
    const int nFunctions = 100;
    const int maxCoeff = 10000;
    std::vector<std::pair<double, double>> functions;
    LinearFunctionsMaximum<double> lfm;
    for (auto i : range(nFunctions)) {
        std::pair<double, double> fn = { std::rand() % (2 * maxCoeff) - maxCoeff, std::rand() % (2 * maxCoeff) - maxCoeff };
        lfm.add_function(fn.first, fn.second);
        functions.push_back(fn);

        for (int x : range(-1000, 1000)) {
            auto bruteForceValue = std::numeric_limits<double>::lowest();
            for (auto fn : functions)
                relax_max(bruteForceValue, fn.first * x + fn.second);
            ASSERT_DOUBLE_EQ(bruteForceValue, lfm.get_value(x));
        }
    }
}